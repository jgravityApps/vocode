...
# when test in local/heroku
_eel_js_file = 'html/assets/js/eel.js'
# when build desktop app
...
@btl.route('/')
def rootIndex():
    return btl.static_file('index.html', root=root_path)

"""@btl.error(404)
def error404(error):
    return btl.static_file('404.html', root=root_path)
"""