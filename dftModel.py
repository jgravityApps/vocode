...
def dftAnal(x, w, N):
	...
	X = fft(fftbuffer)                                      # compute FFT
	absX = abs(X[:hN])                                      # compute ansolute value of positive side
	absX[absX < np.finfo(float).eps] = np.finfo(float).eps  #absX[absX < np.finfo(float).eps] = np.finfo(float)s    # if zeros add epsn to handle log
	mX = 20 * np.log10(absX)                                # magnitude spectrum of positive frequencies in dB
	
	X[:hN].real[abs(X[:hN].real) < tol] = 0.0            # for phase calculation set to 0 the small values
	X[:hN].imag[abs(X[:hN].imag) < tol] = 0.0            # for phase calculation set to 0 the small values
